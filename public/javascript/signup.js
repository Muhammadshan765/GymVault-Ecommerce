...
const beforeUnloadHandler = (e) => {
    if (document.getElementById("otpModal")?.classList.contains("flex")) {
        e.preventDefault();
        e.returnValue = "You have a pending OTP verification. Are you sure you want to leave?";
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const validatePassword = (password) => {
        return /^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)[A-Za-z\d!@#$%^&*]{8,12}$/.test(password)
            ? { isValid: true }
            : { isValid: false, message: "Password must be 8-12 characters with at least one uppercase letter, one lowercase letter, and one number." };
    };
    ...
    form.addEventListener('submit', async function (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Initially disable and blur resend button
                ...
                const timerInterval = setInterval(() => {
                    ...
                    if (timeLeft < 0) {
                        ...
                        // Enable and unblur resend button
